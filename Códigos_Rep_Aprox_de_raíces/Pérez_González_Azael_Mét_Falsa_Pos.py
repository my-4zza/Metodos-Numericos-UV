...
def falsa_posicion():
    ...
    if f(x_0) * f(x_1) >= 0:
        print("Error: f(x_0) y f(x_1) deben tener signos opuestos.")
        return

    x_i_plus_1_old = 0
    
    # Inicializa en infinito para garantizar la entrada al cálculo de tolerancia.
    error = float('inf') 
    iteracion = 1
    ...
    # Ciclo principal del método de la falsa posición.
    while True:
        # Fórmula del método de la falsa posición para encontrar la intersección.
        x_i_plus_1 = x_1 - (f(x_1) * (x_0 - x_1)) / (f(x_0) - f(x_1))
